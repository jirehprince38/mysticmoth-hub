...
if game.PlaceId ~= 13864667823 then
	...
	-- Free Gamepasses (LOBBY)
		...
		local Window = OrionLib:MakeWindow({
			Name = "MysticmMoth Hub",
			HidePremium = true,
			SaveConfig = true,
			ConfigFolder = "MysticmMothHub",
			IntroEnabled = true,
			IntroText = "MysticmMoth Hub • Loading...",
...
-- Tables
	...
	Tab:AddLabel("MysticmMoth Hub • Game Breaking")
	Tab:AddLabel("⚡ Fast controls • Use carefully")
	...
	local Tab = Window:MakeTab({
		Name = "Teleports",
		Icon = "rbxassetid://6031075933",
		PremiumOnly = false
	})

	Tab:AddLabel("Smooth Teleport Menu")
	local Section = Tab:AddSection({
		Name = "Locations"
	})
	...
	Tab:AddButton({
		...
		Callback = function()
			if SemiGodmode == false then
				...
				Notify('Warning', "Damaging Yourself Won't Work Due To Having 'Semi Godmode' Enabled!", 'rbxassetid://4483345998', 7)
			end
		end
	})
	...
	Notify('MysticmMoth Hub', "Loaded successfully!\nUI refreshed • v1.1.0\nOriginated by Credits To LeoDicap On The Old V3rmillion.", 'rbxassetid://4483345998', 8)
	ScriptLoaded = true
	OrionLib:Init()
end
